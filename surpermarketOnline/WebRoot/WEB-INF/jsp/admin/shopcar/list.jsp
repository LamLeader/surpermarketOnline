...
<html>
	<body>
		<display:table name="list"
			...
			<display:column title="商品" property="productName"/>
			<display:column title="数量" property="num"/>
						<c:choose>
...
<c:if test="${row.productId=='productName' }"></c:if>
			</display:column>
		<c:if test="${1==1 }"></c:if>
		</display:table>
	</body>
	<script type="text/javascript">
	function updateColumnproductId(productId,id){
		if(!confirm("确定要更新为"+productId+"吗?")){
			return false;
		}
		var params={id:id,productId:productId};
		$.post("updateColumnproductId.html",params,function(
				result){
			result=eval("("+result+")");
