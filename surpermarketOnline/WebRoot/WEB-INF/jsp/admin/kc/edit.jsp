	...
	<form id="form1" name="form1" action="" method="post">
		...
		<center>
			<table class="mobile" style="width: 95%;">
				...
				<tr height="25">
					<td class="outDetail" style="width: 30%">商品： <label
						style="font-weight: bold; color: red"> </label></td>
					<td class="outDetail2"><select id="productId" name="productId">
							<c:forEach items="${productList }" var="lists">
								<option value="${lists.id }" ${map.productId==lists.id?'selected':'' }>${lists.productName
									}</option>
							</c:forEach>
					</select></td>
				</tr>
				<tr height="25">
					<td class="outDetail" style="width: 30%">添加库存数量： <label
						style="font-weight: bold; color: red"> </label></td>
					<td class="outDetail2"><input id="kcNum" type="text"
						value="${map.kcNum }" name="kcNum" /></td>
				</tr>
				<c:if test="${1==1 }"></c:if>
